...
class LocalVariableRenamer(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        # 1. Collect all names to be renamed (from args and local vars)
        ...
        # 2. Directly modify the argument names in the function signature
        for arg in node.args.args:
            if arg.arg in rename_map:
                arg.arg = rename_map[arg.arg]

        # 3. Apply the renames to all variable usages within the function body
        applier = NameApplier(rename_map)
        # We need to transform each statement in the body
